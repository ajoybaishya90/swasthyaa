...
<link href="../assets/css/anm/forw_patient.css" rel="stylesheet">
        ...
        <div id="sidebar">
            <ul class="nav list-group">
                ...
                <li class="active" style="border-bottom : 1px solid #9fa1a3;">
                    <a href="anm_pro.php"><img src="../assets/image/pw.png" id="profileIcon">&nbsp;&nbsp;&nbsp; Profile</a>              
                </li>
            </ul>
        </div>
    </div>
...
function getPersonalInfo(pid){
	$.ajax({
		...
		success: function(data){ 
            for(var i in data){
				$('.tbody').append('<tr><td colspan="2" style="text-align : left; font-size : 25px; color : #f1635d;">'+data[i].adminName+'</td></tr><tr><td colspan="2" style="text-align : left; font-weight : bold;">'+data[i].adminDesignation+'</td></tr><tr><td colspan="2" style=""><hr /></td></tr><tr><td style="text-align : left; font-weight : bold;">Mobile Number</td><td style="text-align : left">'+data[i].adminMobile+'</td></tr><tr><td style="text-align : left; font-weight : bold;">Health Facility</td><td style="text-align : left">'+data[i].hname+'</td></tr><tr><td style="text-align : left; font-weight : bold;">Address</td><td style="text-align : left">'+data[i].adminAddress+'</td></tr><tr><td style="text-align : left; font-weight : bold;">District</td><td style="text-align : left">'+data[i].adminDistrict+'</td></tr><tr><td colspan="2" style="text-align : left;"><input type="button" class="btn" value="Edit Details" onclick="updateInfo('+data[i].aid+');"></td></tr>');
			}
		}		
	});
}
...
/*function updateInfo(id){
	...
	$.ajax({
		type: "POST",
		url: 'model/getAdminInfo.php',
		data: { pid : id },
		dataType: 'JSON',
		success: function(data){ 
            for(var i in data){
				$('#updateName').val(data[i].Name);
				$('#updateDesignation').val(data[i].Designation);
				$('#updateMobile').val(data[i].Contact_No);
				$('#updateEmail').val(data[i].Email);
				$('#updateAddress').val(data[i].Address);
				$('#updateDistrict').val(data[i].District);
				$('#updateState').val(data[i].State);
			}
		}	
	});	
}*/
