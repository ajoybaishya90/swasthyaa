<?php
	...
	if(strlen($patientNumber) != 0){
		$selectQuery = "Select * from patient_personal_info where Patient_Mobile=:mobile";
		$result = $conn->prepare($selectQuery);
		$result->bindParam(":mobile", $patientNumber);
		$result->execute();
		$count = $result->rowCount();
		if($count == 0){
			$query = "INSERT INTO patient_personal_info (Unique_ID , Name , DOB , Age , Sex , Address , Area , District , State , PIN , Patient_Mobile , Family_Mobile , CHW_ID , ASHA_ID , Status , entry_date , Arrival_status , referenceStatus) 
				VALUES  (:Unique_ID , :Pname , :dob , :age , :sex , :address , :area , :district , :state , :pin , :patient_mobile , :family_mobile , :chw_id , :asha_id  ,:status , :date , :astatus , :refstatus)";
			...
			if($r){
				...
				header("location: ../new_patient.php?successPersonalInfo");
			}else{
				header("location: ../new_patient.php?failPersonalInfo");
			}
		}else{
			header("location: ../new_patient.php?exist");
		}
	}else{
		$query = "INSERT INTO patient_personal_info (Unique_ID , Name , DOB , Age , Sex , Address , Area , District , State , PIN , Patient_Mobile , Family_Mobile , CHW_ID , ASHA_ID , Status , entry_date , Arrival_status , referenceStatus) 
		VALUES  (:Unique_ID , :Pname , :dob , :age , :sex , :address , :area , :district , :state , :pin , :patient_mobile , :family_mobile , :chw_id , :asha_id  ,:status , :date , :astatus , :refstatus)";
