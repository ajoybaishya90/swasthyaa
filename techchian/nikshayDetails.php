...
function getNikshayDetails(pid){
	$.ajax({
		type: "POST",
		data: { pid : pid },
		url: 'model/getNikshayDetails.php',
		dataType: 'JSON',
		success: function(data){
		    for(var i in data){
				$('#establishmentId').val(data[i].nikshay_id);
				$('#nid').val(data[i].cdl);
				$('#rntpc').val(data[i].rntpc);
				$('#pmdt').val(data[i].pmdt);
				$('#drid').val(data[i].drtb);
				$('#tb_unit').val(data[i].tb_unit);
			}
		}		
	});
}
...
function enableFields(){
	$('#establishmentId').prop("disabled",false);
	$('#nid').prop("disabled",false);
	$('#rntpc').prop("disabled",false);
	$('#pmdt').prop("disabled",false);
	$('#drid').prop("disabled",false);
	$('#tb_unit').prop("disabled",false);
}


function updateNikshayDetails(){
	...
	$.ajax({
		...
		url: 'model/updateNikshayDetails.php',
		dataType: 'JSON',
		success: function(data){
		    	if(data.result == 'Success'){
					...
					if(pstatus == '1'){
						window.location.href = "http://localhost/swasthyaa/techchian/symptom.php?id="+pid;
					}
				}else{
        ...
        <div id="main">
				...
				<div class="col-md-8 col-lg-8">
					<div class="form-group" style="margin-top : 20px;">
						...
						<div class="form-group">
							...
							<input type="text" name="establishmentId" id="establishmentId" class="form-control" placeholder="" disabled>
						</div>
			
						<div class="form-group">
							<label>CDL NIKSHAY ID</label>
							<div class="form-inline">
								<input type="text" name="nid" id="nid" class="form-control" placeholder="" disabled>
							</div>
						</div>
			
						<div class="form-group">
							<label>RNTPC TB Reg No.</label>
							<input type="text" name="rntpc" id="rntpc" class="form-control" placeholder="" disabled>
						</div>
			
						<div class="form-group">
							<label>PMDT TH Number</label>
							<input type="text" name="pmdt" id="pmdt" class="form-control" placeholder="" disabled>
						</div>
			
						<div class="form-group">
							<label>DR TB NIKSHY ID</label>
							<input type="text" name="drid" id="drid" class="form-control" placeholder="" disabled>
						</div>	
					</div>
				</div>
			</div>
			<div class="row" style="margin-left : 40px;">
				...
				<div class="col-md-3 col-lg-3" style="">
					<div class="form-group">
						...
						<input type="text" name="tb_unit" id="tb_unit" class="form-control" placeholder="" disabled>
					</div>
				</div>
			</div>
			<div class="row" style="margin-top : 20px; margin-bottom : 40px;">
				<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8" style="text-align : center;">					
						...
						<input type="button" class="btn btn-primary" value="EDIT" onclick="enableFields();">
						<input type="button" class="btn btn-primary" value="NEXT" onclick="updateNikshayDetails();">					
				</div>
			</div>
        </div>                        
    </div>
