...
$(document).ready(function() {
	...
	pid = $('#pId').val();
	checkPatientNormalTBStatus(pid);
	checkPatientMDRStatus(pid);
	checkPatientXDRStatus(pid);
	
});

function checkPatientNormalTBStatus(pid){
	$.ajax({
		type: "POST",
		url: 'model/checkPatientNormalTBStatus.php',
		data: { pid : pid },
		dataType: 'JSON',
...
function checkPatientMDRStatus(pid){
	$.ajax({
		type: "POST",
		url: 'model/checkPatientMDRStatus.php',
		data: { pid : pid },
		dataType: 'JSON',
		success: function(data){ 
            if(data.result == 'Absent'){
...
function checkPatientXDRStatus(pid){
	$.ajax({
		type: "POST",
		url: 'model/checkPatientXDRStatus.php',
		data: { pid : pid },
		dataType: 'JSON',
		success: function(data){ 
            if(data.result == 'Absent'){
...
function showDiagnosis(){
	...
	if(value == 'normal'){
		if(pstatus == '0'){
			window.location.href = "http://localhost/swasthyaa/techchian/normaltbdiagnosis.php?id="+pid;
		}
		if(pstatus == '1'){
			window.location.href = "http://localhost/swasthyaa/techchian/normaltbfollowup.php?id="+pid;
		}
	...
	if(value == 'mdr'){	
		if(mdrstatus == '0'){
			window.location.href = "http://localhost/swasthyaa/techchian/mdrdiagnosis.php?id="+pid;	
		}
		if(mdrstatus == '1'){
			window.location.href = "http://localhost/swasthyaa/techchian/mdrfollowup.php?id="+pid;
		}
	...
	if(value == 'xdr'){
		if(xdrstatus == '0'){
			window.location.href = "http://localhost/swasthyaa/techchian/xdrtbdiagnosis.php?id="+pid;
		}
		if(xdrstatus == '1'){
			window.location.href = "http://localhost/swasthyaa/techchian/xdrfollowup.php?id="+pid;
		}
        ...
        <div id="main">
			...
			<div class="row" style="margin-left : 40px;">
				<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
					<div class="form-group" style="margin-top : 20px;">
						...
						<table class="table">
							<tbody>
								<tr><td style="width : 250px;">Normal TB - Diagnosis</td><td><input type="radio" name="ptype" value="normal" checked></td></tr>
								<tr><td>MDR TB - Drug Susceptibility</td><td><input type="radio" name="ptype" value="mdr"></td></tr>
								<tr><td>XDR TB - Presumtive H monopoly</td><td><input type="radio" name="ptype" value="xdr"></td></tr>
							</tbody>
						</table>
					</div>
				</div>
			...
			<div class="row" style="margin-top : 20px;">
				<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6" style="text-align:center;">					
						...
						<input type="button" class="btn btn-primary" value="NEXT" onclick="showDiagnosis();">
				</div>
			</div>			
        </div>                        
    </div>
